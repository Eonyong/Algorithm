...
def BFS(r, c, n, boards, visited):
    ...
    while temp:
        ...
        for j, i in [[-1, 0], [0, 1], [1, 0], [0, -1]]:
            nj, ni = row + i, col + j
            if 0 <= nj < n and 0 <= ni < n and not visited[nj][ni] and (
                    not boards[nj][ni] or boards[r][c] == boards[nj][ni]):
                visited[nj][ni] = True
                if not boards[nj][ni]:
                    cnt += 1
                    ...
                    if len(temp_group) >= 2:
                        if len(group) < len(temp_group):
                            ...
                            if cnt > zero_cnt:
                                ...
                                if group[0][0] < temp_group[0][0]:
                                    group, zero_cnt = temp_group, cnt
                                elif group[0][0] == temp_group[0][0]:
                                    if group[0][1] < temp_group[0][1]:
                                        group, zero_cnt = temp_group, cnt
    if len(group) < 2:
        break
    answer += len(group) ** 2
